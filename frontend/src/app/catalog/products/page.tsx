...
const getProductGroupKey = (p: any) => {
  ...
  // Strip dimensions and size values to find the base model
  let cleanName = name
    .replace(/\b\d+\s*[xх\*×]\s*\d+\s*[xх\*×]\s*\d+\b/gi, "")
    .replace(/\b\d+(?:\.\d+)?\s*[xх\*×]\s*\d+(?:\.\d+)?\b/gi, "")
    .replace(/\b(400|600|700|800|900)\b/g, "")
    .replace(/\s+/g, " ")
    .trim();
  
  cleanName = cleanName.replace(/\s\d+мм/gi, "");
  
  const img = p.image_url || "";
  return `${brand}_${cleanName.toLowerCase()}_${img}`;
};

function CatalogProductsContent() {
  const searchParams = useSearchParams();
  const categoryParam = searchParams.get("category");
  const isCatalogMode = true;
    ...
    const fetchProducts = async () => {
      try {
        ...
        const url = selectedCategoryId
          ...
          : `/api/v1/products/?group=true&t=${Date.now()}`;
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Group products to prevent duplicate cards (same model/color, differing only by size)
    // Sort by price ascending first to keep the cheapest variant as the card representative
            ...
            {loading ? (
              ...
              <>
                <Link 
                  ...
                  className={cn("block w-full text-left px-3 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider transition-colors", !selectedCategoryId ? "bg-blue-50 dark:bg-blue-900/30 text-[#2c3b6e] dark:text-blue-400" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50")}
                >
                  Все товары
                </Link>
                {mainCategories.map(cat => {
                  ...
                  return (
                    <Link 
                      key={cat.id} 
                      href={catHref} 
                      className={cn("block w-full text-left px-3 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider transition-colors", selectedCategoryId === cat.id ? "bg-blue-50 dark:bg-blue-900/30 text-[#2c3b6e] dark:text-blue-400" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50")}
                    >
                      {cat.name.replace(/\sMAFF$/i, '').toUpperCase()}
                    </Link>
                  );
                })}
              </>
        ...
        <div className="mb-8">
           <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white mb-6 pb-2 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
              ...
              {selectedThicknesses.length > 0 && (
                <span className="bg-[#2c3b6e] dark:bg-blue-600 text-white w-4 h-4 rounded-full flex items-center justify-center text-[9px]">{selectedThicknesses.length}</span>
              )}
           </h3>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[oklch(0.27_0.06_267.62)] transition-colors duration-300 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="sticky top-[64px] lg:top-[160px] z-20 bg-white/95 dark:bg-[oklch(0.27_0.06_267.62)]/95 backdrop-blur-md pb-4 border-b border-slate-100 dark:border-white/5">
          {isCatalogMode ? (
            <h1 className="text-2xl lg:text-3xl font-black text-[#2c3b6e] dark:text-blue-400 uppercase tracking-tight">
              {loading && categoryParam ? (
          ...
          {!loading && isLaminateCategorySelected && (
            <div className="mt-6 flex items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3 rounded-2xl border border-slate-100 dark:border-white/5">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Толщина:</span>
                ...
                {["8 мм", "10 мм", "12 мм"].map(thickness => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "px-4 py-1.5 rounded-full border text-[11px] font-bold transition-all",
                        isSelected
                          ? "bg-[#2c3b6e] text-white border-[#2c3b6e] dark:bg-blue-600 dark:border-blue-600"
                          : "bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:border-slate-400 dark:hover:border-white/30"
                      )}
                    >
                      {thickness}
                    </button>
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-start mt-8">
          {/* Sidebar */}
          {!selectedCategoryId && (
            <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-[260px] max-h-[calc(100vh-280px)] overflow-y-auto pr-4 no-scrollbar">
               {FilterContent}
            </aside>
          )}
