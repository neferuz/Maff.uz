...
async def main():
    ...
    async with async_session() as session:
        ...
        res = await session.execute(text("SELECT description, price, is_active, stock, unit, brand_id, country, brand FROM product WHERE id = 7020"))
        p = res.fetchone()
        if p:
            new_name = "Паркет 3-х сл T&G Дуб ШМ 15.00x190.00х600-2100мм ШМ Черектер Молочный Шоколад"
            
            insert_query = """
                INSERT INTO product (name, description, price, category_id, image_url, is_active, stock, unit, brand_id, country, brand)
