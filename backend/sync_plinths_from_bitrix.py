...
async def fetch_bitrix_plinths():
...
async def main():
    bitrix_prods = await fetch_bitrix_plinths()
    ...
    async with engine.begin() as conn:
        ...
        skirting_cat_ids = [12] + get_all_descendants(12)
        
        res_prods = await conn.execute(text(f"""
            SELECT id, name, sku, category_id, image_url, is_active 
            FROM product 
            WHERE category_id IN ({','.join(map(str, skirting_cat_ids))})
        """))
        pg_prods = res_prods.fetchall()
        print(f"Fetched {len(pg_prods)} plinth products from PostgreSQL.")
        
        matched_count = 0
        updated_ids = []
        
        print("\nMatching and updating plinths...")
        for p in pg_prods:
            matched = False
            best_match = None
            ...
            # 1. Match by SKU exactly (case-insensitive)
            if p_sku:
            ...
            # 2. Match by clean name (case-insensitive, Cyrillic support)
            if not matched:
                p_name_clean = clean_name(p.name)
                for bp in bitrix_prods:
                    bp_name_clean = clean_name(bp["name"])
                    if len(bp_name_clean) > 8 and len(p_name_clean) > 8:
                        if p_name_clean == bp_name_clean or p_name_clean in bp_name_clean or bp_name_clean in p_name_clean:
                            best_match = bp
                            matched = True
                            break
            ...
            if matched and best_match["image_path"]:
                ...
                print(f"MATCH & UPDATE: PG ID={p.id} | Name='{p.name}' | SKU='{p.sku}'  ==>  Image='{url}'")

        # 3. For any plinth product (active=True) that still has NO photo, set is_active = False (archive it)
        print("\nArchiving plinths without photos...")
        no_photo_query = f"""
        ...
        if to_archive:
            print(f"Found {len(to_archive)} plinth products without photos to archive.")
            ...
            print(f"Successfully deactivated {result.rowcount} plinth product(s).")
        else:
            print("No plinths without photos to archive.")
