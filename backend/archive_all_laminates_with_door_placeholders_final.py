...
async def main():
    ...
    async with engine.begin() as conn:
        ...
        # Query active products under these categories with door placeholders in image_url
        query = f"""
            SELECT id, name, sku, category_id, image_url
            FROM product
            WHERE category_id IN ({','.join(map(str, all_laminate_cat_ids))})
            AND is_active = True
            AND (image_url LIKE '%zadoor%' OR image_url LIKE '/images/products/%')
        """
        ...
        if to_archive:
            print(f"\nFound {len(to_archive)} active laminate product(s) with door placeholders to archive:\n")
            for p in products:
                print(f"  ID={p.id} | SKU={p.sku} | Cat={category_names.get(p.category_id, p.category_id)} | '{p.name}' | Image={p.image_url}")
                
            print(f"\nDeactivating {len(to_archive)} product(s)...")
            ...
            print("\nNo active laminate products with door placeholders found.")
