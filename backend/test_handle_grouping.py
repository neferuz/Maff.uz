...
def clean_handle_name(name: str) -> str:
    if not name:
        return ""
    ...
    # Casing normalization for display or grouping
    # Standardize brands
    cleaned = re.sub(r'^agb\b', 'AGB', cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r'^system\b', 'System', cleaned, flags=re.IGNORECASE)
    
    # Uppercase first word if it is alphabetic (for models like VEGA, SINUS)
    words = cleaned.split()
    if words:
        # Uppercase all words that are fully english letters
        words = [w.upper() if re.match(r'^[a-z]+$', w) else w for w in words]
        cleaned = " ".join(words)
