...
async def main():
    ...
    async with engine.begin() as conn:
        ...
        print("\nUpdating matched products in PostgreSQL database...")
        for p in pg_prods:
            matched = False
            best_match = None
            ...
            # 1. Match by extracted Decor Code
            if p_decor:
            ...
            # 2. Fallback to clean name substring match
            if not matched:
            ...
            if matched and best_match["image_path"]:
                ...
                # Update database
                query = "UPDATE product SET image_url = :url, is_active = True WHERE id = :id"
                await conn.execute(text(query), {"url": url, "id": p.id})
                updated_count += 1
                
        print(f"\nSuccessfully updated and activated {updated_count} laminate products in the database!")
