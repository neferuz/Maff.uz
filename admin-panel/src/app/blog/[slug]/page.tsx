    ...
    <div className="relative space-y-6 animate-in fade-in duration-700 pb-12">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-[#e3e8ee]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1 text-[10px] font-semibold text-[#4f566b]">
              <Link href="/blog" className="hover:text-[#2c3b6e] transition-colors">Блог</Link>
              <ChevronRight className="w-2.5 h-2.5 text-[#a3acb9]" />
              <span className="text-[#1a1f36]">{isNew ? "Новая статья" : "Редактор"}</span>
            </div>
            <h1 className="text-[16px] font-bold text-[#1a1f36] tracking-tight leading-none mt-0.5">
              {isNew ? "Создание статьи" : data.title || "Редактирование"}
            </h1>
          </div>
          {isDirty && (
        ...
        <div className="flex items-center gap-3">
          {!isNew && (
            <a 
              ...
              target="_blank"
              className="flex items-center gap-2 px-3 py-2 text-[11px] font-semibold text-[#4f566b] bg-white border border-[#e3e8ee] rounded-lg hover:bg-[#f7f8f9] transition-all"
            >
              <Eye className="w-3.5 h-3.5 text-[#a3acb9]" />
              Предпросмотр
            </a>
          )}
          <button 
         ...
         <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white border border-[#e3e8ee] rounded-xl p-4 space-y-4">
               ...
               <div className="space-y-1">
                  <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Заголовок статьи</label>
                  <textarea 
                    value={data.title} 
                    onChange={(e) => setData({...data, title: e.target.value})} 
                    rows={2}
                    placeholder="Заголовок, который зацепит читателя..."
                    className="w-full px-3 py-2 bg-[#f7f8f9] border border-[#e3e8ee] rounded-lg text-[14px] font-bold text-[#1a1f36] outline-none focus:bg-white focus:border-[#2c3b6e] transition-all resize-none" 
                  />
               </div>

               {isNew && (
                  <div className="space-y-1">
                     <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Адрес (Slug)</label>
                     <div className="flex items-center bg-[#f7f8f9] border border-[#e3e8ee] rounded-lg px-3 py-2">
                        <span className="text-[#a3acb9] text-[13px] font-medium mr-1">post-</span>
                        <input 
                          value={currentSlug} 
               ...
               <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Краткий анонс</label>
                  <textarea 
                    value={data.excerpt} 
                    onChange={(e) => setData({...data, excerpt: e.target.value})} 
                    rows={3}
                    placeholder="Пара предложений для превью в списке статей..."
                    className="w-full px-3 py-2 bg-[#f7f8f9] border border-[#e3e8ee] rounded-lg text-[13px] font-medium text-[#1a1f36] outline-none focus:bg-white focus:border-[#2c3b6e] transition-all resize-none leading-relaxed" 
                  />
               </div>
            </div>
            ...
            <div className="space-y-5">
               <div className="flex items-center justify-between px-2">
                  <div className="flex items-center gap-2">
                     ...
                     <h3 className="text-[13px] font-bold text-[#1a1f36] tracking-wider">Контент статьи</h3>
                  </div>
                  <div className="flex gap-2">
                     <button onClick={() => addSection("text")} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-[#e3e8ee] text-[#2c3b6e] hover:bg-[#f7f8f9] rounded-lg transition-all text-[11px] font-semibold">
                        <Plus className="w-3 h-3" /> Текст
                     </button>
                     <button onClick={() => addSection("image")} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-[#e3e8ee] text-[#2c3b6e] hover:bg-[#f7f8f9] rounded-lg transition-all text-[11px] font-semibold">
                    ...
                    <div key={idx} className="bg-white border border-[#e3e8ee] rounded-xl p-5 relative group transition-all hover:border-[#2c3b6e]/20">
                       <div className="flex items-center justify-between mb-4 pb-3 border-b border-[#f7f8f9]">
                          <div className="flex items-center gap-2 text-[#4f566b]">
                             ...
                             <span className="text-[10px] font-semibold tracking-wider">Блок {idx + 1} — {section.type === "text" ? "Текстовый" : "Изображение"}</span>
                          </div>
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                             ...
                             <button onClick={() => removeSection(idx)} className="p-1.5 text-[#a3acb9] hover:text-[#cd5c5c] hover:bg-[#cd5c5c]/5 rounded-md transition-all">
                       ...
                       {section.type === "text" ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-[1fr_200px] gap-6 items-start">
                             <div className="space-y-4">
                                <div className="space-y-1">
                                   <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Ссылка на изображение</label>
                                   <input value={section.url} onChange={(e) => updateSection(idx, "url", e.target.value)} className="w-full bg-[#f7f8f9] border border-[#e3e8ee] rounded-lg px-3 py-2 text-[12px] outline-none focus:bg-white focus:border-[#2c3b6e]" placeholder="https://..." />
                                </div>
                                <div className="space-y-1">
                                   <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Подпись к фото</label>
                                   <input value={section.caption} onChange={(e) => updateSection(idx, "caption", e.target.value)} className="w-full bg-transparent border-none p-0 text-[12px] font-semibold text-[#1a1f36] italic opacity-60 outline-none" placeholder="Добавьте описание..." />
                                </div>
                             </div>
                             <div className="aspect-square bg-[#f7f8f9] rounded-xl border border-dashed border-[#e3e8ee] flex flex-col items-center justify-center relative overflow-hidden group/upload">
                                ...
                                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/upload:opacity-100 transition-opacity flex items-center justify-center">
                                   <p className="text-[10px] font-semibold text-white tracking-wider">Загрузить</p>
                                </div>
         ...
         <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white border border-[#e3e8ee] rounded-xl p-4 space-y-4">
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-[13px] font-bold text-[#1a1f36] tracking-wider">Параметры</h3>
               </div>
               
               <div className="space-y-2">
                  <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Обложка статьи</label>
                  <div className="aspect-[16/10] bg-[#f7f8f9] rounded-xl border border-[#e3e8ee] flex flex-col items-center justify-center relative overflow-hidden group/cover">
                     ...
                     <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/cover:opacity-100 transition-opacity flex items-center justify-center">
                        <p className="text-[10px] font-semibold text-white tracking-wider">Изменить обложку</p>
                     </div>
                     <input type="file" onChange={(e) => handleFileUpload(e, "main")} className="absolute inset-0 opacity-0 cursor-pointer" />
                  </div>
                  <input value={data.image} onChange={(e) => setData({...data, image: e.target.value})} className="w-full bg-[#f7f8f9] border border-[#e3e8ee] rounded-lg px-3 py-2 text-[11px] outline-none focus:bg-white focus:border-[#2c3b6e]" placeholder="Или URL..." />
               </div>

               <div className="space-y-4 pt-2">
                  <div className="space-y-1.5">
                     <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Категория</label>
                  ...
                  <div className="space-y-1">
                     <label className="text-[10px] font-semibold text-[#4f566b] tracking-wider">Дата публикации</label>
                     <div className="relative">
            ...
            {/* Quick Tips */}
            <div className="bg-[#f7f8f9] border border-[#e3e8ee] rounded-xl p-4 space-y-3">
               <div className="flex items-center gap-2 mb-2">
                  <Sparkles className="w-4 h-4 text-[#2c3b6e]" />
                  <h3 className="text-[13px] font-bold text-[#1a1f36] tracking-wider">Рекомендации</h3>
               </div>
            ...
            {/* Support */}
            <div className="bg-white border border-[#e3e8ee] rounded-xl p-4">
               <div className="flex items-center gap-2 mb-3">
                  <User className="w-4 h-4 text-[#2c3b6e]" />
                  <h3 className="text-[13px] font-bold text-[#1a1f36] tracking-wider">Авторство</h3>
               </div>
               ...
               <div className="pt-3 border-t border-[#e3e8ee]">
                  <span className="text-[10px] font-semibold tracking-wider text-[#a3acb9] italic">Version 2.4.0</span>
               </div>
            </div>
         </div>
